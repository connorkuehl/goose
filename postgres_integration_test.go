...
func TestIntegrationPostgres(t *testing.T) {
	...
	t.Run("Autocompletions", func(t *testing.T) {
		resetDB(t, db)

		feeds := &Feeds{DB: db}
		subscriptions := &Subscriptions{db: db}

		url1, err := url.Parse("https://go.dev/blog/feed.atom")
		if err != nil {
			t.Fatalf("url.Parse [%q]: %v", "https://go.dev/blog/feed.atom", err)
		}
		feed1, err := feeds.Create(url1, time.Time{})
		if err != nil {
		...
		url2, err := url.Parse("https://kubernetes.io/feed.xml")
		if err != nil {
			t.Fatalf("url.Parse [%q]: %v", "https://kubernetes.io/feed.xml", err)
		}
		feed2, err := feeds.Create(url2, time.Time{})
		if err != nil {
			t.Fatalf("Failed to create second feed: %v", err)
		}

		_, err = subscriptions.Create(feed1.ID, "server1", "channel1", "The Official Go Blog", time.Time{})
		if err != nil {
			t.Fatalf("Failed to create first subscription: %v", err)
		}

		_, err = subscriptions.Create(feed2.ID, "server1", "channel2", "The Kubernetes RSS Feed", time.Time{})
		if err != nil {
		...
		tests := []struct {
			...
			{name: "match both", input: "the", want: []string{"The Official Go Blog", "The Kubernetes RSS Feed"}},
			{name: "match one", input: "kub", want: []string{"The Kubernetes RSS Feed"}},
			{name: "match none", input: "Rust", want: []string{}},
		}

		for _, tt := range tests {
			t.Run(tt.name, func(t *testing.T) {
				got, err := subscriptions.GetCollectionNameAutocompletionsForServer("server1", tt.input)
				if err != nil {
					t.Errorf("want err=<nil>, got err=%v", err)
					return
				}

				gotComparable := make(map[string]struct{})
				wantComparable := make(map[string]struct{})

				for _, g := range got {
					gotComparable[g] = struct{}{}
				}
				for _, w := range tt.want {
					wantComparable[w] = struct{}{}
				}
